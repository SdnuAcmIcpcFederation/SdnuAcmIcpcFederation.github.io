...
在具体讲何为「记忆化搜索」前，先来看如下的例题：
 [NOIP 2005 采药](https://www.luogu.com.cn/problem/P1048)
...
$mem_{pos,tleft} = \max\{mem_{pos+1,tleft-tcost(pos)}+mget(pos),mem_{pos+1,tleft}\}$
...
$dp(i,j,k) = dp(i+1,j+1,k-a_j) + dp(i+1,j,k)$
